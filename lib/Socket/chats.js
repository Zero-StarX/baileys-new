...
const makeChatsSocket = (config) => {
    ...
    const suki = usync_1.makeUSyncSocket(config);
    const { ev, ws, authState, generateMessageTag, sendNode, query, onUnexpectedError, groupFetchAllParticipating } = suki;
    ...
    const onWhatsApp = async (...jids) => {
        ...
        const results = await suki.executeUSyncQuery(usyncQuery);
        if (results) {
        ...
        await authState.keys.transaction(async () => {
            ...
            while (collectionsToHandle.size) {
                ...
                for (const key in decoded) {
                    ...
                    try {
                        if (snapshot) {
                            const { state: newState, mutationMap } = await Utils_1.decodeSyncdSnapshot(
                                name, 
                                snapshot, 
                                getAppStateSyncKey, 
                        ...
                        if (patches.length) {
                            const { state: newState, mutationMap } = await Utils_1.decodePatches(
                                name, 
                                patches, 
                                states[name], 
                                getAppStateSyncKey, 
    ...
    const appPatch = async (patchCreate) => {
        ...
        if (config.emitOwnEvents) {
            ...
            const { mutationMap } = await Utils_1.decodePatches(
                name, 
